/**
 * Ngân hàng câu hỏi trò chơi Kéo Co Tri Thức (60+ câu hỏi đa dạng)
 */
...
const QUESTIONS_STORAGE_KEY = 'keo_co_tri_thuc_custom_bank_v2';
...
// Xáo trộn các đáp án A, B, C, D của câu hỏi
...
// Tải câu hỏi từ localStorage hoặc ngân hàng mặc định
function loadQuestionBank() {
  try {
    ...
    console.error('Không thể đọc ngân hàng câu hỏi từ localStorage', e);
  }
...
function saveQuestionBank(bank) {
  try {
    ...
    console.error('Lỗi khi lưu câu hỏi vào localStorage', e);
  }
}

// Sinh 2 bộ 10 câu hỏi độc lập riêng biệt cho Đội 1 và Đội 2
function generateQuestionSets(customBank) {
  let pool = customBank && customBank.length > 0 ? [...customBank] : [...DEFAULT_QUESTION_BANK];
